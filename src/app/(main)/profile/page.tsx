...
export default function ProfilePage() {
  ...
  return (
    <div className="max-w-2xl mx-auto pb-24 md:pb-6">
      ...
      {/* Stats Grid - Responsive: 2 cols on mobile, 4 cols on tablet+ */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.1 }}
        className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 mb-6 px-4"
      >
        ...
        <div className="bg-gray-50 rounded-2xl p-3 sm:p-4 text-center">
          <div className="text-xl sm:text-2xl font-bold">{uniqueRestaurants}</div>
          <div className="text-xs text-gray-500">Places</div>
        </div>
        <div className="bg-gray-50 rounded-2xl p-3 sm:p-4 text-center">
          <div className="flex items-center justify-center gap-1">
            <Flame className="w-4 h-4 text-gray-700" />
            <span className="text-xl sm:text-2xl font-bold text-gray-900">7</span>
          </div>
          <div className="text-xs text-gray-500">Streak</div>
        </div>
        <div className="bg-gray-50 rounded-2xl p-3 sm:p-4 text-center">
          <div className="text-xl sm:text-2xl font-bold">{avgCalories}</div>
          ...
          {userProfile?.badges && userProfile.badges.length > 0 ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
              {userProfile.badges.map((badge) => (
                <div
                  key={badge.id}
                  className="text-center p-3 sm:p-4 bg-gray-50 rounded-2xl"
                >
